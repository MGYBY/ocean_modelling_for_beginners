...
g = 9.81
f = 1.e-4
r = 0.0 ! bottom friction dispabled
rho = 1028.0
...
! initialise bathymetry with random perturbations
...
DO k = 0,nx+1
DO j = 1,ny
  hzero(j,k) = 10.0 + (ran3(ist)-0.5)*0.1
END DO
  hzero(0,k) = 0.0
  hzero(ny+1,k) = 0.0
END DO
...
DO j = 0,ny+1
 hzero(j,0) = hzero(j,nx)
 hzero(j,nx+1) = hzero(j,1)
END DO
...
DO j = 0,ny+1
DO k = 0,nx
  term1 = (u(j,k)+UGEO(j)+abs(u(j,k)+UGEO(j)))*dt/dx
  term2 = (u(j,k+1)+UGEO(j)+abs(u(j,k+1)+UGEO(j)))*dt/dx
  CuP(j,k) = 0.25*(term1+term2)
  term1 = (u(j,k)+UGEO(j)-abs(u(j,k)+UGEO(j)) )*dt/dx
  term2 = (u(j,k+1)+UGEO(j)-abs(u(j,k+1)+UGEO(j)))*dt/dx
  CuN(j,k) = 0.25*(term1+term2)
  CvP(j,k) = 0.25*(v(j,k)+v(j,k+1)+abs(v(j,k))+abs(v(j,k+1)))*dt/dy
  CvN(j,k) = 0.25*(v(j,k)+v(j,k+1)-abs(v(j,k))-abs(v(j,k+1)))*dt/dy
  Cu(j,k) = 0.5*abs(u(j,k)+u(j,k+1)+2.*UGEO(j))*dt/dx 
...
DO k = 0,nx+1
  term1 = (u(j,k)+UGEO(j)  +abs(u(j,k)  +UGEO(j)  ) )*dt/dx
  term2 = ( u(j+1,k)+UGEO(j+1)+abs(u(j+1,k)+UGEO(j+1)) )*dt/dx
  CuP(j,k) = 0.25*(term1 + term2)
  term1 = ( u(j,k)  +UGEO(j)   - abs(u(j,k)  +UGEO(j)  ) )*dt/dx
  term2 = ( u(j+1,k)+UGEO(j+1) - abs(u(j+1,k)+UGEO(j+1)) )*dt/dx
  CuN(j,k) = 0.25*(term1 + term2)
  ...
  Cu(j,k) = 0.5*abs(u(j,k)+u(j+1,k)+UGEO(j)+UGEO(j+1))*dt/dx
...
DO j = 0,ny+1
DO k = 0,nx+1
  CuP(j,k) = 0.5*(u(j,k)+UGEO(j)+abs(u(j,k)+UGEO(j)))*dt/dx
  CuN(j,k) = 0.5*(u(j,k)+UGEO(j)-abs(u(j,k)+UGEO(j)))*dt/dx
  CvP(j,k) = 0.5*(v(j,k)+abs(v(j,k)))*dt/dy
  CvN(j,k) = 0.5*(v(j,k)-abs(v(j,k)))*dt/dy
  Cu(j,k) = abs(u(j,k)+UGEO(j))*dt/dx 
...
DO j = 0,ny+1
DO k = 0,nx+1
  CuP(j,k) = 0.5*(un(j,k)+UGEO(j)+abs(un(j,k)+UGEO(j)))*dt/dx
  CuN(j,k) = 0.5*(un(j,k)+UGEO(j)-abs(un(j,k)+UGEO(j)))*dt/dx
  CvP(j,k) = 0.5*(vn(j,k)+abs(vn(j,k)))*dt/dy
  CvN(j,k) = 0.5*(vn(j,k)-abs(vn(j,k)))*dt/dy
  Cu(j,k) = abs(un(j,k)+UGEO(j))*dt/dx
