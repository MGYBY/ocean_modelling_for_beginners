...
// This script produces a graph showing float trajectories in the top layer
...
trx1=read("TRx1.dat",-1,600); try1=read("TRy1.dat",-1,600); 
uin=read("u1.dat",-1,51); vin=read("v1.dat",-1,51);

x = (0:1:50); y = (0:1:50); // location vectors  
[ntot nx] =size(hin);
ntot = floor(ntot/nx); time = ntot*2/24; n = ntot;

jtop = (n-1)*51+1; jbot = jtop+50; 
u = uin(jtop:jbot,1:51); // grab respective data block
v = vin(jtop:jbot,1:51); // grab respective data block

// average velocity components at scalar gridpoint
uh = u; vh =v;
...
if speed(j,k) < 0.15 ; uh(j,k) = 0; vh(j,k) = 0; end;end;end;

// interpolate flow field onto coarser grid
ua(1:26,1:26) = 0.; va(1:26,1:26) = 0.0; // scaling 
...
champ(xa,ya,10*ua',10*va',1); // vector plot
...
xstring(30.0,50.5,"time = "+string(int(10*time)/10)+" 5 days"); //add time
